...
async def assign_dataset_to_annotating(project_id: str, dataset_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Set labeled_images to 1 if it's currently 0 (to move from Unassigned to Annotating)
        if dataset.labeled_images == 0 and dataset.total_images > 0:
            # Update labeled_images to 1 to indicate annotation work has started
            # This will move the dataset from "Unassigned" to "Annotating" section
...
async def rename_dataset(project_id: str, dataset_id: str, new_name: str = Body(..., embed=True), db: Session = Depends(get_db)):
    ...
    try:
        ...
        try:
            old_folder_path = os.path.join("..", "uploads", project.name, old_dataset_name)
            new_folder_path = os.path.join("..", "uploads", project.name, new_name)
            ...
            if os.path.exists(old_folder_path) and not os.path.exists(new_folder_path):
                ...
                print(f"DEBUG: Dataset folder rename skipped - old exists: {os.path.exists(old_folder_path)}, new exists: {os.path.exists(new_folder_path)}")
...
async def delete_dataset(project_id: str, dataset_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        try:
            dataset_folder_path = os.path.join("..", "uploads", project.name, dataset_name)
            print(f"DEBUG: Attempting to delete dataset folder: '{dataset_folder_path}'")
            print(f"DEBUG: dataset_folder_path exists: {os.path.exists(dataset_folder_path)}")
            
            if os.path.exists(dataset_folder_path):
                shutil.rmtree(dataset_folder_path)
                print(f"Deleted dataset folder: {dataset_folder_path}")
            else:
                print(f"DEBUG: Dataset folder not found: {dataset_folder_path}")
